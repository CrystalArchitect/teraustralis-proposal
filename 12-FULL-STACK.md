...
Each layer states what exists in Australia today, what the southern node adds, and its honest status. Statuses are load-bearing: *operating* means it runs today; *product-not-industry* means Australians consume it but none of it is built or hosted here at industrial scale; *proposed* means argued in this document set; *speculative* means an idea on the Roadmap, nothing more.

### 1. Minerals & Feedstock — the base of every layer
...
**Status:** operating (extraction) · proposed (value-add orientation).

### 2. Energy — what feeds compute and propellant alike
...
**Status:** operating (generation, storage precedent) · product-not-industry (manufacturing) · proposed (industrial build-out).

### 3. Connectivity — the binding layer
...
**Status:** product-not-industry · proposed (sovereign-scale ground segment).

### 4. Compute & Intelligence — where the merger points
...
**Status:** proposed (compute) · operating at prototype scale, honestly labelled (CrystalCore).
...
### 6. Autonomy & Robotics — the layer that waits
...
**Status:** speculative, and labelled so on the Roadmap.

### 7. The Far Layer — flagged, not claimed

Self-replicating systems — the Von Neumann probe lineage studied since Dyson and NASA's 1980 summer study — are the logical end of a stack built for multiplanetary distance. One observation belongs to this project and is worth recording: a self-replicating probe is the ultimate local-first system. It must carry its whole constitution with it, because no signal from home arrives in time. Consent, continuity, and fail-safe isolation at interstellar distance — CrystalCore's constraints are, quietly, that machine's operating requirements.

**Status:** dream belt, stated as such. It shapes the architecture's ambitions; it makes no claims on anyone's roadmap.

---

## Why One Geography Matters

A stack scattered across jurisdictions pays integration costs at every border: regulatory seams, logistics seams, security seams. A stack landed in one geography — ore to battery to grid to compute to pad to port — compounds instead. Australia is not the cheapest location for any single layer. It is the rare location where the layers *stack*: minerals beside energy beside ports beside isolation, under one law, in the hemisphere the architecture currently lacks — with the consent and capability frameworks (Documents 06, 10, 11) already written, so integration does not arrive as extraction.
...
*Framework document — vision and structure, honestly labelled. Layer statuses are stated per layer; no company named here has agreed to anything; "present as product, absent as industry" describes market reality, not a commitment by any party to change it.*
